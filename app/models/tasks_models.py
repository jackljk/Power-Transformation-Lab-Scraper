...
class Task(BaseModel):
    ...
    def from_template(cls, prompt: Union[str, Dict[str, Any]], template_name: str = "default",) -> "Task":
        ...
        try: 
            if template_name == "default" or template_name == "pdf_default":
                assert isinstance(prompt, str), "Prompt must be a string for default template"
                task_str = template["task_format"].format(
                    prompt=prompt,
                )
            elif template_name == "tabular_extraction":
                assert isinstance(prompt, dict), "Prompt must be a dictionary for tabular extraction template"
                # handle filter
            ...
            raise ValueError(f"Missing key in Task Model Template: {e}")
